...
for i_sub in (subjects):
    ...
    n_motiv = 3
    ts_tmp = sio.loadmat('../dataBackupMTLnew/cleanData/dataClean-ICA-' + str(i_sub) + '-T1.mat')['dataSorted'][:, :, :,
             :3, 1]  # [N,T,n_trials,motiv]
    ...
    N = 71  # number of channels - you need to set the number by hand
    ...
    'clf', skllm.LogisticRegression(C=10, penalty='l2', multi_class='multinomial', solver='lbfgs', max_iter=500))])
    ...
    rk_FC = np.zeros([n_bands, 2, n_rep, int(N * (N - 1) / 2)],
                     dtype=np.int)  # RFE rankings for FC-type measures (N(N-1)/2 feature)
    pearson_corr_rk = np.zeros(
        [n_bands, n_measures, int(n_rep * (n_rep - 1) / 2)])  # stability of rankings measured by Pearson correlation
    ...
    for i_band in range(n_bands):
        ...
        for i_measure in range(n_measures):
            ...
            if i_measure == 0:  # power of signal within each sliding window (rectification by absolute value)
                ...
                EEG_FC = np.zeros([n_motiv, n_trials, N,
                                   N])  # dynamic FC = covariance or Pearson correlation of signal within each sliding window
                for i_motiv in range(n_motiv):
                    for i_trial in range(n_trials):
                        ...
                        if i_measure == 2:  # correlation, not covariance
                            EEG_FC[i_motiv, i_trial, :, :] /= np.sqrt(
                                np.outer(EEG_FC[i_motiv, i_trial, :, :].diagonal(),
            ...
            for i_rep in range(n_rep):
                for ind_train, ind_test in cv_schem.split(vect_features, labels):  # false loop, just 1
                    ...
                    conf_matrix[i_band, i_measure, i_rep, 0, :, :] += skm.confusion_matrix(y_true=labels[ind_test],
                                                                                           y_pred=c_MLR.predict(
                                                                                               vect_features[ind_test,
                                                                                               :]))
                    ...
                    conf_matrix[i_band, i_measure, i_rep, 1, :, :] += skm.confusion_matrix(y_true=labels[ind_test],
                                                                                           y_pred=c_1NN.predict(
                                                                                               vect_features[ind_test,
                                                                                               :]))
                    ...
                    perf_shuf[i_band, i_measure, i_rep, 0] = c_MLR.score(vect_features[ind_test, :],
                                                                         shuf_labels[ind_test])

                    c_1NN.fit(vect_features[ind_train, :], shuf_labels[ind_train])
                    perf_shuf[i_band, i_measure, i_rep, 1] = c_1NN.score(vect_features[ind_test, :],
                                                                         shuf_labels[ind_test])
        ...
        for i_measure in range(n_measures):
            ...
            for i_rep1 in range(n_rep):
                for i_rep2 in range(i_rep1):
                    pearson_corr_rk[i_band, 0, i_cnt] = \
                    stt.pearsonr(rk_pow[i_band, i_rep1, :], rk_pow[i_band, i_rep2, :])[0]
                    pearson_corr_rk[i_band, 1, i_cnt] = \
                    stt.pearsonr(rk_FC[i_band, 0, i_rep1, :], rk_FC[i_band, 0, i_rep2, :])[0]
                    pearson_corr_rk[i_band, 2, i_cnt] = \
                    stt.pearsonr(rk_FC[i_band, 1, i_rep1, :], rk_FC[i_band, 1, i_rep2, :])[0]
                    i_cnt += 1
